...
request = urllib.request.Request(
    ...
    headers={
        ...
        "Referer": (
            "https://www.iberdrola.es/movilidad-electrica/"
            "productos/puntos-de-recarga"
        ),
        "User-Agent": (
...
print(f"Consultando el punto Iberdrola {PUNTO_ID} ({UBICACION})...")
...
try:
    with open("estado_anterior.txt", "r", encoding="utf-8") as archivo:
        estado_anterior = archivo.read().strip()
except FileNotFoundError:
    estado_anterior = ""

with open("estado_anterior.txt", "w", encoding="utf-8") as archivo:
    archivo.write(estado_actual)

if estado_actual == "LIBRE" and estado_anterior != "LIBRE":
    print("HAY UN CARGADOR DISPONIBLE")
else:
    print("NO HAY UNA NUEVA DISPONIBILIDAD")
...
if estado_actual == "LIBRE" and estado_anterior != "LIBRE":
    if not token or not chat_id:
        print("Aviso no enviado: faltan los secretos de Telegram.")
    else:
        mensaje = (
            "⚡ ¡HAY CARGADOR LIBRE EN IBERDROLA AV. BENISSA, BENIDORM! "
